...
| **swift_app.merge**(input_dir=..., mode=..., basin=..., variable=..., ...) | Merge existing station files into GeoPackages |
| **swift_app.plot**(input_dir=..., mode=..., basin=..., variable=..., cwc=..., ...) | Generate hydrograph plots from existing output |
| **swift_app.cite**() | Print citation |
| **swift_app.coffee**() | Print coffee-break message |
| **swift_app.datasets** | Dataset name constants (e.g. discharge, rainfall) |
| **swift_app.basins**() | List WRIS basin id/name table |
...
| **merge**(mode="wris", basin="Godavari", output_dir="output") | `swift --merge-only --input-dir output --output-dir output -b Godavari -q` (with dataset flags as needed) |
| **merge**(input_dir="output") | `swift --merge-only --input-dir output` |
| **plot**(mode="wris", basin="Godavari", output_dir="output") | `swift --plot-only --input-dir output -b Godavari -q` |
| **plot**(mode="cwc", output_dir="output") | `swift --plot-only --input-dir output --cwc` |
| **cite**() | `swift --cite` |
| **coffee**() | `swift --coffee` |
